...
void TKY_dataProcess (void)
{
    ...
    switch (key_val)
    {
        ...
        case KEY_1_LONG :
            key_val = 1;
            if (!(touchinsflag & 0x0C))//When a pulley instance or linear slider instance is triggered, the current instance is not triggered
            {
                touchinsflag |= 1 << 1;
    ...
    if (!(touchinsflag & 0x06))//When a key instance or a pulley instance is triggered, the current instance is not triggered
    {
