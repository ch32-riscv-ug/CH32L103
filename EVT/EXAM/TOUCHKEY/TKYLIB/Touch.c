...
const TKY_CH_GPIO_S TKY_Pin[ 10 ] = {
...
static void touch_Regcfg (void)
{
    ...
    ADC1->CTLR1 |= (1 << 24) | (1 << 26) | (1<<25);                                                      // Enable ADC Buffer And TouchKey
        ...
        { 
            ...
            while (1)
            {
                ...
                else
                {
                    ...
                    {
                        // chx_mean = TKY_GetCurChannelMean (my_tky_ch_init[ i ].channelNum, charge_time - 1, 16, 20);
...
static uint16_t touch_DetecLineSlider(touch_slider_cfg_t * p_slider)
{
    ...
    if ((uint32_t)p_slider == (uint32_t)NULL)
    {
        return TOUCH_OFF_VALUE;
    }
